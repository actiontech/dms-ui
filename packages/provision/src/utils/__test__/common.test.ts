...
describe('utils/Common', () => {
  test('should check params is a email address', () => {
    ...
    expect(emailValidate([] as any)).toBe(false);
    expect(emailValidate(null as any)).toBe(false);
  });

  test('should format moment to YYYY-MM-DD HH:mm:ss', () => {
    expect(formatTime('2021-06-09T08:11:52+00:00')).toBe('2021-06-09 16:11:52');
    expect(formatTime('2021-06-09T08:11:52+00:00', '--')).toBe(
      '2021-06-09 16:11:52'
    );
    expect(formatTime(undefined, '--')).toBe('--');
  });

  test('should format time to HH', () => {
    expect(timeAddZero(9)).toBe('09');
    expect(timeAddZero(11)).toBe('11');
  });
