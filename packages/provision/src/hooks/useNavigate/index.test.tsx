...
describe('test useNavigate', () => {
  ...
  test('should be added custom prefix', () => {
    ...
    superRender(
      <>
        <Routes>
          <Route path="/" element={<Start />} />
          <Route
            path={`${PROVISION_BASE_URL}search`}
            element={<LocationComponent />}
          />
          <Route
            path={`${PROVISION_BASE_URL}blog`}
            element={
              <>
    ...
    // expect(window.location.pathname).toEqual(`${PROVISION_BASE_URL}blog`);
    expect(screen.getByTestId('location-display')).toHaveTextContent(
      `${PROVISION_BASE_URL}blog`
    );

    fireEvent.click(screen.getByText('click'));

    expect(screen.getByTestId('location-display')).toHaveTextContent(
      `${PROVISION_BASE_URL}search`
    );
