...
import {
  databaseAccountListTableToolbarActions,
  databaseAccountListPageHeaderActions
} from './actions';
import {
...
const DatabaseAccountList = () => {
  ...
  useEffect(() => {
    ...
    if (queries?.filter_keyword) {
      setSearchKeyword(queries.filter_keyword);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const pageHeaderActions = useMemo(() => {
    return databaseAccountListPageHeaderActions(projectID, () =>
      toggleModal(ModalName.DatabaseAccountDiscoveryModal, true)
    );
  }, [projectID, toggleModal]);
  ...
  return (
    <>
      ...
      <PageHeader
        ...
        extra={
          <Space>
            {pageHeaderActions['discover-account']}
            {pageHeaderActions['create-account']}
          </Space>
        }
      />
