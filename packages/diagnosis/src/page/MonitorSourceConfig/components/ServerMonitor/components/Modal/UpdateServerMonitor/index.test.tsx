...
describe('test update server monitor', () => {
  ...
  it('should show error when nothing input', async () => {
    const { baseElement } = superRender(<UpdateServerMonitor />);
    ...
    await act(async () => jest.advanceTimersByTime(1000));
    expect(getAllBySelector('.basic-input-wrapper')?.[0]).toHaveValue('');
  ...
  it('should send request when input correct fields info', async () => {
    ...
    await act(async () => jest.advanceTimersByTime(1000));
    expect(getAllBySelector('.basic-input-wrapper')?.[0]).toHaveValue('test12');
    await act(async () => jest.advanceTimersByTime(3000));
    ...
    await act(async () => jest.advanceTimersByTime(3000));
    expect(
      screen.queryByText('修改服务器监控源test12成功！')
    ).toBeInTheDocument();
    expect(mockDispatch).toBeCalledTimes(1);
    expect(mockDispatch).toBeCalledWith({
      payload: {
