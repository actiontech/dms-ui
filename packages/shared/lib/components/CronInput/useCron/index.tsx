...
const useCron = (options?: CronOptions) => {
  ...
  const updateError = (errorMessage: string | ''): boolean => {
    if (errorMessage !== '') {
      setError(t(errorMessage as any) as string);
      return false;
    }
    if (error !== '') {
      setError('');
  ...
  const updateTime = (hour: number, minute: number) => {
    const hourTempError = checkNumber([hour], CronItemType.Hour);
    const minuteTempError = checkNumber([minute], CronItemType.Minute);
    if (updateError(hourTempError) && updateError(minuteTempError)) {
      setHour([hour]);
      setMinute([minute]);
      updateCronByNumber([minute], [hour], Day, Month, Week);
    }
  };
