...
const DetailReport = () => {
  ...
  const onClickAnalyze = useCallback(
    (data: IAuditPlanReportSQLResV2) => {
      ...
      window.open(
        `/sqle/project/${projectID}/auditPlan/${urlParams.reportId}/${sqlNumber}/${urlParams.auditPlanName}/analyze?instance_name=${auditTask?.data?.audit_plan_instance_name}&schema=${auditTask?.data?.audit_plan_instance_database}`,
        '_blank'
      );
    },
