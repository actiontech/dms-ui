  ...
  const {
        ...
        }),
    {
      onSuccess: (res) => {
        ...
        const allowExecute = (workflowList: IWorkflowDetailWithInstance[]) => {
          // 排除占位工单
          if (
            workflowList.filter((i) => !!i.status && !!i.workflow_id).length ===
            0
          ) {
            return false;
          }
        ...
        stageWorkflowNodes.forEach((node, index) => {
          if (index !== stageWorkflowNodes.length - 1) {
            nodeEdges.push({
              ...
              type: 'buttonEdge',
              data: {
                onRelease: allowRelease(node.data.workflowList ?? [])
                  ? () =>
                      onRelease(node.data.stageId, node.data.workflowList ?? [])
                  : undefined,
                onExecute: allowExecute(node.data.workflowList ?? [])
                  ? () => onExecute(node.data.workflowList ?? [])
                  : undefined
              }
            });
          }
        ...
        setNodes([...stageNameNodes, ...stageWorkflowNodes]);
        setEdges(nodeEdges);
      }
    }
  );
    ...
    <>
      ...
      >
        <VersionDetailStyleWrapper hidden={modifyWorkflowSqlStatement}>
          <PageHeader title={versionDetail?.version} extra={<BackToList />} />
          <Spin
