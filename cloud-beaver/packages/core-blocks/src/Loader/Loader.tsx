...
import { ExceptionMessage } from '../ExceptionMessage/ExceptionMessage.js';
...
export const Loader = observer<Props>(function Loader({
  ...
  const [contextState] = useState<ILoaderContext>(() => ({
    state: observable(new Set<string>())
  }));
    ...
    <LoaderContext.Provider value={contextState}>
      <>
        ...
        <div
          ...
          className={s(
            style,
            { loader: true, loaderOverlay: overlay, small, fullSize, inline },
            className
          )}
        >
          <Spinner
            className={s(style, {
              spinner: true,
              spinnerSecondary: secondary || overlay
            })}
