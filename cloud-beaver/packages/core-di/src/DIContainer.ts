...
function logger(planAndResolve: interfaces.Next): interfaces.Next {
  return (args: interfaces.NextArgs) => {
    try {
      ...
      // @ts-ignore
      let metadata: Array<() => void> =
        Reflect.getMetadata('design:paramtypes', args.serviceIdentifier) || [];
      const matchIndex = /argument\s(\d+)/.exec((exception as Error).message);
      const matchDependency = /(serviceIdentifier:|class)\s([\w]+)/.exec(
        (exception as Error).message
      );
      let index = parseInt(matchIndex?.[1] ?? '-1');
      const dep = metadata.find(
        (service) => service.name === matchDependency?.[2]
      );
      ...
      function getName(element: any | (() => void)) {
        return typeof element === 'function' && 'name' in element
          ? element.name
          : String(element);
      }
      ...
      const dependenciesList = metadata.map(
        (value, i) => `${i} - ${getName(value)}`
      );
