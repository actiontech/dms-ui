...
export class Task<T> implements ITask<T> {
  ...
  then<TResult1 = T, TResult2 = never>(
    ...
    onRejected?:
      | ((reason: any, cancelled: boolean) => TResult2 | PromiseLike<TResult2>)
      | null
      ...
      queueMicrotask(() => {
        ...
        this.executeValue(
          ...
          (reason: any) => {
            this.resolvePromise(
              task,
              onRejected(reason, cancelled ?? false) as TResult2
            );
    ...
    onRejected?:
      | ((reason: any, cancelled: boolean) => TResult | PromiseLike<TResult>)
      | null
  ): Task<T | TResult> {
  ...
  executeTasks<T>(
    tasks: ((value: T, cancelled?: boolean) => void)[],
    value: T,
    cancelled: any
  ) {
    this.fulfillCallbacks = [];
  ...
  private registerCancel(cancel: TaskCancelFn): void {
    ...
    this.cancelCallback = (reason) =>
      Task.resolve(copy(reason)).then(() => cancel(reason));
  }
  ...
  static allSettled<T extends readonly unknown[] | []>(
    ...
    -readonly [P in keyof T]:
      ...
      | { status: 'rejected' | 'cancelled'; reason: any };
  }> {
    return new Task((resolve) => {
      let counter = 0;
      const result = [] as {
