...
import {
  ExecutorInterrupter,
  type IExecutorInterrupter
} from './ExecutorInterrupter';
import type {
  IExecutionContext,
...
} from './IExecutorHandlersCollection';
import type { ISyncExecutor } from './ISyncExecutor';

export class SyncExecutor<T = void>
  extends ExecutorHandlersCollection<T>
  implements ISyncExecutor<T>
{
  ...
  execute(
    ...
    scope?:
      | IExecutorHandlersCollection<T>
      | Array<IExecutorHandlersCollection<T>>
    ...
    return this.executeHandlersCollection<T>(
      this,
      data,
      context,
      [scope || []].flat()
  ...
  private executeHandlersCollection<T>(
    ...
    const interrupter = context.getContext(
      ExecutorInterrupter.interruptContext
    );
  ...
  private executeChain<T>(
    ...
    for (const link of collection.chain.filter((link) => link.type === type)) {
      ...
      this.executeHandlersCollection(
        ...
        mappedData,
        chainedContext,
        [collection.getLinkHandlers(link.executor) || []].flat()
      );
    }
  }
  ...
  protected override executeHandlerWithInitialData(
    handler: IExecutorHandler<T>
  ) {
