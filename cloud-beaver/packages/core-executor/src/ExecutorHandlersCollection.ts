...
export class ExecutorHandlersCollection<T = void, TResult = any | Promise<any>>
  implements IExecutorHandlersCollection<T, TResult>
{
  ...
  private readonly links: Map<
    IExecutorHandlersCollection<any, TResult>,
    IExecutorHandlersCollection<T, TResult>
  >;
  ...
  removeBefore(executor: IExecutorHandlersCollection<any, TResult>): void {
    this.chain = this.chain.filter(
      (link) => link.executor !== executor || link.type !== 'before'
    );
  }
  ...
  removeNext(executor: IExecutorHandlersCollection<any, TResult>): void {
    this.chain = this.chain.filter(
      (link) => link.executor !== executor || link.type !== 'next'
    );
  }
  ...
  protected executeHandlerWithInitialData(
