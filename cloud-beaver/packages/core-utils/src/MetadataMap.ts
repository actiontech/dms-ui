...
export type MetadataValueGetter<TKey, TValue> = (
  key: TKey,
  metadata: MetadataMap<TKey, any>
) => TValue;
export type DefaultValueGetter<TKey, TValue> = (
  key: TKey,
  metadata: MetadataMap<TKey, TValue>
) => TValue;
...
export class MetadataMap<TKey, TValue> implements Map<TKey, TValue> {
  ...
  constructor(
    private readonly defaultValueGetter?: DefaultValueGetter<TKey, TValue>
  ) {
    this.syncData = null;
  ...
  get(
    key: TKey,
    defaultValue?: DefaultValueGetter<TKey, TValue>,
    ...
    if (invalidate) {
      ...
      if (!provider) {
        throw new Error(
          'MetadataMap: defaultValue should be provided if defaultValueGetter not set'
        );
      }

      const val = provider(key, this);
      const isNotPrimitiveValue = typeof val === 'object' && val !== null;

      this.temp.set(key, isNotPrimitiveValue ? observable(val) : val);
    }
